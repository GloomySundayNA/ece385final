...
BYTE SPI_wr(BYTE data) {
	...
	wdata = data;
	int return_value = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &wdata, 1, &rdata, 0);
	if (return_value < 0) {
		printf("ERROR SPI_wr\n");
	}
	return rdata;
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//if return code < 0 print an error
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	alt_u8 write_buffer[2];

	 write_buffer[0] = reg + 2;
	 write_buffer[1] = val;

	 int return_value = alt_avalon_spi_command(SPI_0_BASE, 0, 2, write_buffer, 0, &rdata, 0);
	 if (return_value < 0)
		 printf("ERROR MAXreg_wr rv1 %d\n", return_value);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	alt_u8 write_buffer[nbytes + 1];
	write_buffer[0] = wdata;
	for(int i = 1; i < nbytes + 1; i++) {
			write_buffer[i] = data[i - 1];
	}
	int return_value = alt_avalon_spi_command(SPI_0_BASE, 0, 1 + nbytes, write_buffer, 0, &rdata, 0);

	if (return_value < 0)
	printf("ERROR MAXbytes_wr rv1 %d\n", return_value);
	return &(data[nbytes]);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
	BYTE ret_val;
	//MAXbytes_rd(reg, 1, &ret_val);
	wdata = reg;
	int return_value = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &wdata, 1, &ret_val, 0);
	if (return_value < 0)
		printf("ERROR MAXreg_rd rv1 %d\n", return_value);
	return ret_val;
}
//multiple-byte write
//returns a pointer to a memory position after last written
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	wdata = reg;
	int return_value = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &wdata, nbytes, data, 0);
	if (return_value < 0)
		printf("ERROR MAXbytes_rd rv1 %d\n", return_value);
	return &data[nbytes];
}
